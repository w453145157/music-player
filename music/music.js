...
function getlyric(){
	...
	$.post('http://api.jirengu.com/fm/getLyric.php', {sid: Sid})
        .done(function (lyr){
                ...
                if(line != ""){
                    for(var i in line){//遍历歌词数组
                        ...
                        for(j in time){//遍历时间数组
                            var t = time[j].slice(1, -1);
                            var t1 = t.split(':');//分析时间  时间的格式是[00:00.00] 分钟和毫秒是t[0],t[1]
                            //把结果做成数组 result[0]是当前时间，result[1]是纯歌词
                            console.log(t1);
                            var timeArr = parseInt(t1[0], 10) * 60 + parseFloat(t1[1]); //计算出一个curTime s为单位
                            console.log(timeArr);
        	...
        	$('.music-lyric .lyric').html("<li>获取不到本歌曲歌词</li>");
        })
}
...
function showLyric(){
    var liH = $(".lyric li").outerHeight(); //每行高度
    for(var i=0;i< lyricArr.length;i++){//遍历歌词下所有的li
        ...
        if ((curTime > curT) && (curT < nexT)){//当前时间在下一句时间和歌曲当前时间之间的时候 就渲染 并滚动
            ...
            $('.music-lyric .lyric').css('top', -liH*(i-4));
        }
    }
...
$(".basebar").mousedown(function(ev){  //拖拽进度条控制进度
	var posX = ev.clientX;
	var targetLeft = $(ev.target).offset().left;
